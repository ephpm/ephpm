...
Restart the server. PHP can now connect to `127.0.0.1:3306` with `pdo_mysql` — it thinks it's MySQL, but it's actually a single-file SQLite database. See [Database guide](/architecture/database/) for the details.
...
- **Configure** — see [Configuration](configuration/) for the structure of `ephpm.toml`, then [Reference → Configuration](/reference/config/) for every key.
- **Migrate** — coming from Apache or Nginx? [Migration guides](/migration/) show the equivalent ePHPm config.
- **Cluster** — [Clustering setup](/guides/clustering-setup/) walks through gossip + clustered SQLite.
