...
The `security` profile enables the first five analyzers below (`composer-audit`, `semgrep-php`, `malware-yara`, `dangerous-sinks`, `suppression-scan`). `phpstan`, `psalm-taint`, `progpilot`, `phpcs`, `phpmd`, `rector`, and `opcode-scan` are registered but **opt-in** — they run only when named explicitly in `analyzers.enable`, so the default gate behaviour is unchanged. `opcode-scan` additionally **requires a PHP-linked build** of ePHPm (the release binaries; a stub build reports it as skipped — list it in `analyzers.required` to make that skip gate instead).
...
analyzers:
  enable: null             # explicit analyzer list; null = use the profile.
                           ...
                           # opcode-scan (needs a PHP-linked build).
