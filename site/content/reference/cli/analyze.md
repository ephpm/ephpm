...
| `composer-scripts` | native (offline), **opt-in** | supply-chain | confirmed + suspected mix | nothing (reads the target's `composer.json`) | **Opt-in — not in the default `security` set.** The blind spot `composer-audit` leaves: `composer-audit --no-scripts` never inspects the `scripts` block, which runs **arbitrary shell commands** on every `composer install`/`update` (`post-install-cmd`, `post-autoload-dump`, `post-update-cmd`, …) — a supply-chain RCE vector. Pure-native: parses the root `composer.json` (no `composer` CLI, no network, no new dependency — unlike `composer-audit`, which shells out to `composer`) and flags each dangerous command shape: **confirmed / high** `composer-scripts/pipe-to-shell` (a remote fetch piped into a shell, `curl … | sh`/`bash`/`php` — the clearest install-time RCE); **suspected / high** `composer-scripts/remote-fetch` (a `curl`/`wget`/`fetch` of a URL); **suspected / medium** `composer-scripts/eval` (`php -r`, `eval`, `base64_decode`) and `composer-scripts/shell-exec` (raw shell metacharacters `;`, `&&`, `|`, backticks, `$(`, `chmod +x`, `rm -rf`). One finding per command, at its most dangerous shape, anchored at `composer.json` with the offending event named in the message. The legitimate forms are **not** flagged: Composer callbacks (`Vendor\Pkg::method`), the `@php`/`@composer`/`@putenv` directives (`@php artisan migrate`), and plain single-binary invocations (`phpunit`). A tree with no `composer.json` yields no findings and no error; an unparseable `composer.json` is an error (fail-closed). |
...
| `clamav` | external, **opt-in** | malware | confirmed | `clamscan` on `PATH` (ClamAV brings its own signature database — keep it fresh with `freshclam`) | **Opt-in — not in the default `security` set.** Runs `clamscan --recursive --infected --no-summary --stdout <target>` and maps each `<path>: <Signature> FOUND` line to a `high` finding (`rule_id = clamav/<Signature>`). Complements `malware-yara`: ClamAV's freshclam feed (plus any add-on webshell/PHP-malware signatures) catches families a hand-rolled ruleset never enumerates. Exit `0` = clean, `1` = infected (parsed), `2`/other = scan error → gates (fail-closed). `analyzers.clamav_db` optionally points at an alternate signature DB (`clamscan -d`); a *configured but missing* DB path is an error, not a skip. Uses the standalone scanner, so no running `clamd` daemon is needed. |
...
| `obfuscation-scan` | native, **opt-in** | malware | suspected | nothing | **Opt-in — not in the default `security` set.** A native token pass over `*.php` / `*.phtml` that catches the *shape* of obfuscated / backdoor PHP rather than a signature — complementing `clamav`/`malware-yara` by catching a **novel** webshell the signature feeds have never seen. Emits: `obfuscation-scan/eval-of-decode` (high — an `eval`/`assert`/`create_function` argument that decodes data: base64/gzinflate/gzuncompress/rot13/hex2bin/pack/uudecode or `\xNN` escapes), `eval-of-request` (high — an `eval`/`assert`/`system`/`exec`/`shell_exec`/`passthru`/`popen`/`proc_open` argument referencing a request superglobal or `php://input`), `preg-replace-eval` (high — `preg_replace` with the `/e` modifier), `dynamic-call` (medium — a variable function call `$f(...)`, a variable-variable, or `call_user_func`/`call_user_func_array` fed a variable), and `long-encoded-blob` (medium — a run of ≥ 200 base64/hex characters). Because it reasons over source text it cannot see through comments and string literals, so matches there are false positives — hence every finding is `suspected` (a review hotspot, never a hard deny), the same trade `dangerous-sinks` makes. An opcode-backed **confirmed** tier (eval-of-decoded / eval-of-tainted-argument on the opcode stream, building on `opcode-scan`) is **planned — not yet implemented**. |
| `weak-crypto` | native, **opt-in** | security | suspected | nothing | **Opt-in — not in the default `security` set.** A native token pass over `*.php` / `*.phtml` flagging insecure cryptography (CWE-327/328/330), one finding per match anchored at the line. Emits: `weak-crypto/tls-verify-disabled` (high — `CURLOPT_SSL_VERIFYPEER` set to `false`/`0`, `CURLOPT_SSL_VERIFYHOST` set to `0`/`false`, or a Guzzle `'verify' => false`; disabling TLS verification defeats HTTPS, hence high), `weak-crypto/ecb-mode` (medium — ECB block-cipher mode: the `MCRYPT_MODE_ECB` constant or an OpenSSL cipher string ending `-ecb` such as `'aes-256-ecb'`), `weak-crypto/mcrypt` (medium — any `mcrypt_*()` call; the extension is removed in PHP 7.2+ and insecure), `weak-crypto/insecure-random` (medium — `rand()`, `mt_rand()`, or `uniqid()`; not cryptographically secure, though frequently used for non-security purposes), and `weak-crypto/weak-hash` (low — `md5()` or `sha1()`; frequently legitimate for checksums/cache keys, so low). Because it reasons over source text it cannot see through comments/strings, and it cannot confirm whether a `md5()`/`rand()` use is security-relevant — so **every finding is `suspected`** (a review hotspot, never a hard deny), the same trade `dangerous-sinks` makes. Pure Rust, no external tool or new dependency. |
| `secrets-scan` | native, **opt-in** | security | confirmed + suspected mix | nothing | **Opt-in — not in the default `security` set.** A line pass over `*.php` / `*.phtml` flagging hardcoded credentials, one finding per match anchored at the line. Two tiers: **confirmed / high** for self-identifying secret shapes — AWS access key ids (`secrets-scan/aws-access-key-id`), PEM `PRIVATE KEY` blocks (`secrets-scan/private-key`), GitHub tokens (`secrets-scan/github-token`), Slack tokens (`secrets-scan/slack-token`), Google API keys (`secrets-scan/google-api-key`), Stripe live keys (`secrets-scan/stripe-live-key`), JWTs (`secrets-scan/jwt`) — and **suspected / medium** `secrets-scan/generic-high-entropy` for a secret-like variable/array-key/const name (`password`, `secret`, `api_key`, `token`, `client_secret`, …) assigned a string literal that is both ≥ 16 chars and high Shannon entropy (≥ 3.5 bits/char), which rules out `$password = ''` and `$apiKey = 'changeme'`. The secret value is **never** echoed — confirmed findings quote only the (public) vendor prefix (`AKIA…`); the generic finding names only the offending variable. Pure Rust, no external tool or new dependency. |
...
| `writable-exec` | native (offline, path-driven), **opt-in** | malware | confirmed + suspected mix | nothing | **Opt-in — not in the default `security` set.** Detects the *result* of a compromise rather than a source vulnerability: executable PHP (`*.php`/`*.phtml`/`*.php5`/`*.phar`) living where only uploaded data belongs — a dropped webshell. It complements the content scanners (`clamav`, `malware-yara`, `obfuscation-scan`) by flagging a file purely by **location**, so a novel webshell no signature has seen is still caught. The file is never read. Two tiers: **confirmed / high** `writable-exec/uploads-php` for a file anywhere under `wp-content/uploads/` (the canonical webshell-drop location — a *fact* about the tree, so confirmed, and the **only** rule that runs by default); **suspected / medium** `writable-exec/writable-dir-php` for a file under an operator-listed `analyzers.writable_exec_dirs` directory. The defaults are deliberately narrow because some frameworks legitimately write `.php` into data dirs — Laravel compiles Blade to `.php` under `storage/framework/views/`, and WordPress page-cache plugins write `.php` under `wp-content/cache/` — so those are **not** flagged unless an operator opts them in via `writable_exec_dirs` (accepting the `suspected` false-positive trade). Findings anchor at the file (no line). A tree with nothing suspicious yields no findings and no error. Pure Rust, no external tool or new dependency. |
...
analyzers:
  ...
  clamav_db: null          # optional alternate ClamAV signature DB for the
                           # opt-in clamav analyzer (clamscan -d; relative to
                           # the target). null = ClamAV's system database.
                           # Missing path = hard error (fail-closed).
  ...
  writable_exec_dirs: null # extra data-only dirs for the opt-in writable-exec
                           # analyzer (relative to the target). Executable PHP
                           # under any becomes a suspected finding. null =
                           # only the confirmed wp-content/uploads/ rule runs.
                           # Frameworks that write .php into data dirs (Laravel
                           # storage/framework/views, wp-content/cache) are NOT
                           # flagged unless listed here.
...
Shipped: the aggregator, the fail-closed policy engine (strictness levels, confidence model), the config/gating surface, baselines, diff-aware scanning, the incremental cache, operator-only suppressions with tenant-suppression detection, both output formats, the five default-profile analyzers above, and the opt-in `clamav`, `phpstan`, `psalm-taint`, `progpilot`, `phpcs`, `phpmd`, `rector`, `opcode-scan`, `php-lint`, `wp-vuln`, `composer-scripts`, `obfuscation-scan`, `secrets-scan`, `writable-exec`, and `weak-crypto` analyzers (`opcode-scan` — compile-to-opcode sink detection via the embedded engine — is the first slice of [engine-backed analysis](/roadmap/opcode-analysis/); it coexists with `dangerous-sinks` for now, and `php-lint` reuses the same compile path for an in-process `php -l` syntax check). `phpcs`/`phpmd`/`rector` are external-tool quality wrappers (`rector` runs strictly in `--dry-run`, never modifying code). `wp-vuln` is a native, offline supply-chain analyzer that matches installed WordPress plugin/theme/core versions against a downloaded Wordfence Intelligence feed — the WordPress counterpart to `composer-audit`; it is opt-in today, a candidate for the default `security` profile once a first-class feed-refresh workflow ships. `composer-scripts` is a native, offline, dependency-free supply-chain analyzer that parses the root `composer.json`'s `scripts` block and flags dangerous install-time commands — pipe-to-shell (`curl … | sh`, confirmed), remote fetch, inline `eval`/`php -r`, and raw shell — the RCE vector `composer-audit --no-scripts` never inspects; it is opt-in today. `obfuscation-scan` is a native source-text heuristic that catches the *shape* of obfuscated / backdoor PHP (decode-and-execute chains, execution of request input, `preg_replace /e`, dynamic calls, long encoded blobs) — complementing the signature scanners by catching a novel webshell; its findings are `suspected` because a source-text pass cannot see through comments/strings. `secrets-scan` is a native, dependency-free per-file pass that flags hardcoded credentials — self-identifying vendor tokens (AWS/GitHub/Slack/Google/Stripe keys, PEM private-key blocks, JWTs) as confirmed findings and high-entropy secret-like assignments as suspected ones, never echoing the secret value; it is opt-in today. `writable-exec` is a native, offline, path-driven analyzer that flags executable PHP living in a data-only directory (a dropped webshell) by *location* rather than contents — confirmed under `wp-content/uploads/`, suspected under operator-listed `writable_exec_dirs` — complementing the content scanners; it is opt-in today. `weak-crypto` is a native, dependency-free source-text pass that flags insecure cryptography (CWE-327/328/330) — disabled TLS certificate verification (high), ECB block-cipher mode, the removed `mcrypt` extension, non-cryptographic randomness (`rand`/`mt_rand`/`uniqid`), and weak hashes (`md5`/`sha1`, low); every finding is `suspected` because a token pass cannot confirm the security context, and it is opt-in today. Planned — not yet implemented: an opcode-backed `confirmed` tier for `obfuscation-scan` (eval-of-decoded / eval-of-tainted-argument on the opcode stream, building on `opcode-scan`), superglobal→sink taint tracking on the opcode stream, engine-in-the-loop detonation of suspicious inputs (the `engine:` section), and ePHPm-specific rules. The `Analyzer` trait, finding shape, policy engine, and output formats are designed to be stable across those additions.
