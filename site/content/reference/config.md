...
| `code_bundle` | string | `"off"` | **Experimental (POC).** In-memory code bundle: a transparent, read-only overlay that answers `.php` source discovery, reads, and `stat` from RAM so the code path never touches the filesystem (apps need no changes — `require`/`file_exists`/`filemtime` on ordinary paths are just answered faster; misses fall through to disk). `"off"` (**default**) = no bundle, zero overhead. `"scan"` = scan `document_root` at startup and index every `.php` file, **overlay semantics**: a hit is served from RAM, a miss still falls through to a real `stat`. `"sealed"` = the same scan **plus authoritative negatives inside the subtrees named by `code_bundle_sealed_paths`** — a `.php` path under a declared sealed root that is not in the index is reported missing from RAM with *no syscall*. **`code_bundle_sealed_paths` is empty by default, so `"sealed"` on its own behaves exactly like `"scan"`.** Sealing is where the win is (a PSR-4 autoloader's probes are mostly misses) and it is a **correctness change, not just a speed knob**: inside a sealed root it removes the overlay's "a miss falls through, so a runtime-created file still works" guarantee. Everything else — non-`.php` files, anything outside a sealed root (including the rest of `document_root`), uploads, sessions, runtime state — behaves exactly like `scan`. Failures are loud and self-limiting: `include`/`require` and source opens always confirm a negative against disk before returning it, and any write into a sealed root or any negative that turns out to be wrong **permanently disarms that root** (it degrades to `scan` semantics for the life of the process) with a `WARN` naming the path; `code_bundle_verify_negatives` extends that confirmation to the hot probes. The scan runs on a background thread and is published in one atomic step — until it completes, code reads fall through to disk exactly as with `off`, and a sealed root is never armed before its scan is complete. Rejected by `ephpm dev` with a startup error (the bundle freezes source and mtimes, so edits would be invisible). Startup logs which semantics are active. Reclaims the Windows filesystem-metadata tax (~50 µs/`stat`). Pair with `opcache_validate_timestamps = false` — startup **warns** if the bundle is on with validation still on. Single-docroot only; ignored (treated as `off`) under `[server] sites_dir`, with a startup warning. An unknown value (e.g. the not-yet-implemented `"file"`) is a startup error. Env: `EPHPM_PHP__CODE_BUNDLE=sealed`. |
| `code_bundle_max_bytes` | u64 (bytes) | `268435456` (256 MiB) | Hard cap on the bytes the code bundle holds resident in RAM. If indexing would exceed it, ePHPm **refuses to build the bundle** (no partial bundle is installed) and falls through to disk, logging a WARN — bounds the blast radius of pointing `scan` at a pathological tree. With compression on, the *compressed* resident size counts. Ignored when `code_bundle = "off"`. Env: `EPHPM_PHP__CODE_BUNDLE_MAX_BYTES=536870912`. |
| `code_bundle_sealed_paths` | string[] | `[]` | Subtrees the sealed index may answer **authoritative negatives** for, relative to `document_root` (absolute paths accepted but must resolve inside it). **Empty by default, and an empty list makes `code_bundle = "sealed"` behave exactly like `"scan"`** — the half of the feature that can change an answer rather than just its latency needs this second, explicit opt-in. Recommended: `["vendor"]`. That is where the probes worth eliminating are, and it is disjoint from every tree a framework writes into at runtime (`var/cache`, `bootstrap/cache`, `storage/framework/views`), so the failure mode is removed by construction rather than detected later. An entry resolving outside `document_root` is a **hard startup error**. Authority is a one-way latch: a write into a sealed root, or a negative that turns out to be wrong, permanently disarms that root. Ignored (with a startup warning) unless `code_bundle = "sealed"`. Env: `EPHPM_PHP__CODE_BUNDLE_SEALED_PATHS=["vendor"]`. |
| `code_bundle_verify_negatives` | bool | `false` | **Diagnostic.** Confirm every `code_bundle = "sealed"` authoritative negative against the real filesystem and log a `WARN` naming any path the index was wrong about. Sealed mode's whole point is answering "does not exist" with no syscall; if something wrote a `.php` file under the document root after startup, that answer is *wrong* and the symptom (a class or template that mysteriously cannot be found) is hard to attribute. This gives back the syscalls in exchange for naming the path. Source opens and `include`/`require` resolution are confirmed **regardless** of this setting — a wrong answer there is fatal; this extends confirmation to the hot `is_file`/`file_exists`/`stat` probes too. Not a production setting: with it on, sealed mode costs about the same as `"scan"`. Ignored (with a startup warning) unless `code_bundle = "sealed"`. Env: `EPHPM_PHP__CODE_BUNDLE_VERIFY_NEGATIVES=true`. |
| `code_bundle_compression` | string | `"none"` | Optional compression for bundled source in RAM: `"none"`/`"gzip"`/`"zstd"`/`"brotli"`. A codec selects **Model B** — source kept *compressed* in RAM, decompressed on every open (smallest footprint, a per-open CPU cost). `"none"` (default) keeps raw source resident (no per-open cost). Ignored when `code_bundle = "off"`; unknown value is a startup error. Env: `EPHPM_PHP__CODE_BUNDLE_COMPRESSION=zstd`. |
