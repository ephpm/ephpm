...
| `mode` | enum | `"per_request"` | Request-execution model. `"per_request"` (default) = php-fpm-shaped: full `php_request_startup`/`shutdown` per request on ePHPm's dedicated PHP thread pool. `"worker"` = persistent worker mode: boot the framework once per worker, loop over requests (Octane/RoadRunner model); worker-only knobs live in [`[php.worker]`](#phpworker). Any other value — including the pre-v0.10 `"fpm"` — is a **startup error naming the key**, never a silent fallback. Env: `EPHPM_PHP__MODE=worker`. |
...
### Migration from the pre-v0.10 `[php]` keys

The v0.10 rename removed the `spawn_blocking` execution engine (the dedicated pool is the only per-request engine) and collapsed/renamed the concurrency knobs. **Every old key is a startup error naming the key** — an old config fails fast rather than silently dropping your concurrency cap. The mapping:
...
Also new in v0.10: the derived `concurrency` clamps to a floor of **2 on the cgroup-quota path too** (#461) — a `--cpus 1` container used to derive a 1-thread pool, which deadlocks a PHP loopback subrequest. Startup logs an INFO line when the clamp raises the derived value; set `concurrency = 1` explicitly to override.
