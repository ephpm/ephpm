...
- **Per-site override files** (`<[server] site_overrides_dir>/<site-key>.toml`) — a different file, not part of `ephpm.toml`. It understands `document_root` and `auto_prepend_file`; anything else is ignored with a `WARN` naming the file, the site, the keys and a "did you mean" hint. It cannot fail startup because it is read lazily per site, so rejecting the file could only mean discarding `document_root` too — which puts that tenant's whole container back on the web, a worse outcome than ignoring one key. See [Virtual Hosts → Failure modes](/guides/virtual-hosts/#failure-modes-all-of-which-serve-the-container).
...
| `site_overrides_dir` | path | (none) | **Multi-tenant only.** Directory of operator-supplied per-site overrides, one `<site-key>.toml` per virtual host. Exactly two keys: `document_root = "public"` (that site's web root) and `auto_prepend_file = ".preview-env.php"` (a PHP file that site runs before every request — per-request mode only). Both are relative to the **site container** and refused if they resolve outside it. **The directory must live outside `sites_dir`** — startup fails closed otherwise. `open_basedir` is unaffected and stays the site container. Unset disables the mechanism. See [Virtual Hosts → Per-site document root](/guides/virtual-hosts/#per-site-document-root-frameworks-with-a-public-directory) and [Per-site `auto_prepend_file`](/guides/virtual-hosts/#per-site-auto_prepend_file-bootstrap-one-site-before-every-request). |
