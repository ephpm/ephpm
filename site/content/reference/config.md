...
> **Per-site clustered mode: writes are owner-served, and `pdo_mysql` is not covered.** A request that lands on a node which does **not** own the site has its `ephpm_db_query()` / `ephpm_db_execute()` statements forwarded to the site's HRW owner over the cluster channel (already mutually authenticated and encrypted); the owner runs them against its local database, so the write is captured into CDC and replicates to every node. Reads and writes therefore both work on any node, and read-your-writes holds because one node serves both. **The gap:** forwarding is wired into the `ephpm_db_*` bridge only. A stock `pdo_mysql` connection to a non-owner node resolves that node's *local* replica, so its writes are neither forwarded nor replicated — and are discarded when that replica re-bootstraps from the owner. On the per-site clustered path use the [`db-*` drop-in packages](/reference/php-packages/), which call the bridge. This mode is **experimental**: the Turso engine is Beta upstream and per-site CDC layers on top of it.
