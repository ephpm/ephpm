...
(The `jit=` segment shows the resolved `opcache.jit` mode — `tracing` here because these are single-site serve runs on Linux; multi-tenant, worker mode and **Windows** show `jit=disable`, and an explicitly pinned mode carries the `*` marker like every other tunable. Dev mode with the knob unset shows `jit=off (php default)` — no line is emitted at all.)

### OPcache JIT

`ephpm serve` enables the **tracing JIT by default in single-site mode on Linux and macOS**, and disables it everywhere else — including **on Windows**, where PHP's own tracing JIT is currently unsafe (see below). `[php] opcache_jit` overrides the default in any mode. A dedicated startup INFO line always states the effective JIT state and the reason (and risky explicit combinations WARN) — the state is never silent.

| Mode | `opcache_jit` unset | Why |
|------|--------------------|-----|
| Single-site serve on Linux/macOS (no `sites_dir`, `mode = "fpm"`) | **`tracing`** | Measured ~2.4x on CPU-bound PHP (Windows CPU loop 5.56 ms → 2.33 ms); verified working in the ZTS embed serve mode. The single-process embed sidesteps the classic multi-process SHM JIT failure modes. |
| Single-site serve on **Windows** | **`disable`** | Upstream PHP defect, not an ePHPm one. PHP's tracing JIT resolves an opcode handler through `ZEND_FUNC_INFO(exit_info->op_array)` when it compiles a **side trace**; that `op_array` can be a heap copy (a method of a linked class the inheritance cache could not persist into SHM) which is freed at request shutdown while the parent trace's exit info lives on in SHM. Compiling the side trace in a *later* request reads a dangling pointer: `0xC0000005`, no PHP error, process gone. A stock Laravel app dies after **three requests**, deterministically. It reproduces on **stock `php.exe -S`** with no ePHPm involved — 8.5.5 / 8.5.7 / 8.5.9 die, 8.5.4 and 8.4.24 survive 150 requests. Tracked upstream at [php-src PR 21710](https://github.com/php/php-src/pull/21710) (open; regression from php-src PR 21368, first released in 8.5.5). Windows is hit hardest because class linking misses the inheritance cache more often there, but upstream has reproduced the root cause on Linux too. |
...
- **Forcing the tracing JIT on on Windows** works and is the operator's call, but buys the crash described above; startup WARNs. If you want *some* JIT on Windows, use `opcache_jit = "function"` — the function JIT compiles whole hot functions and never builds traces, so it cannot reach the defective side-trace path (verified: 150 requests clean where `"tracing"` dies at 3). Its measured benefit on real web apps is close to zero, though; `disable` is the honest default.
