...
| `fpm_engine` | string | `"pool"` | **fpm mode only.** How a per-request PHP execution is scheduled onto a thread. `"pool"` (**default, v0.9.0+**) runs it on ePHPm's own dedicated OS-thread pool sized by `worker_count` (or, when that is unset, an explicit `workers` — see Migration below) — the pool size is the concurrency cap, so `workers` is bypassed (full queue → 504, closed pool → 503, wedged thread → 504 + replacement). It bounds concurrency, memory, and thread count by construction; makes `overload_policy = "shed"` and `crash_containment` usable without extra config; and measured ~10% higher rps with lower tails than `spawn_blocking` on the Laravel per-request benchmark (3-round A/B, 2026-09-02). `"spawn_blocking"` (**legacy / opt-out**) runs it on tokio's shared blocking pool — unchanged from every release before the pool engine existed; at `workers = 0` it applies no concurrency bound at all (an open-loop flood measured 486 threads / ~1.05 GiB RSS vs the pool's 65 threads / ~193 MiB). **Migration (four things change on upgrade):** (1) an existing `workers = N` with no `worker_count` sizes the pool, so the *concurrency* cap is unchanged (logged at INFO) — but the *queue* behind it becomes bounded (`worker_backlog`, defaulting to the pool size), where `spawn_blocking` queued without limit, so past roughly `2N` in flight requests now hit backpressure instead of being absorbed. (2) A config that already sets `fpm_engine = "pool"` **and** a leftover `workers = N` with no `worker_count` gets a pool of exactly `N`, where it previously got a host-derived size plus a WARN. (3) `crash_containment = true` goes from parsed-but-inert to **armed** (Unix) with no operator action — startup WARNs when it arms. (4) The pool size is derived per host: under a cgroup CPU quota it is `ceil(quota)` with a floor of **1**, so a container limited to 1 vCPU or less derives a one-thread pool with a one-deep queue. That is a hard bound where there previously was none: with the default `overload_policy = "wait"` the third concurrent request parks until `[server.timeouts] request` expires (504), and a PHP script that makes a *blocking* HTTP request back to this same server will deadlock itself until that timeout, because the subrequest needs a pool slot the caller is still holding. On small containers set `worker_count` (and/or `worker_backlog`) explicitly rather than relying on the derivation. An unknown value is a startup error. Ignored in worker mode (startup logs a WARN if `spawn_blocking` cannot apply there). Env: `EPHPM_PHP__FPM_ENGINE=spawn_blocking`. |
...
| `admission` | string | `"fifo"` | Dispatch-admission policy for the bounded queues in front of PHP execution — the worker pool (`mode = "worker"`) and the dedicated fpm pool (`fpm_engine = "pool"`). `"fifo"` (**default**) admits waiters in strict arrival order via a FIFO-fair semaphore — removes the multi-lap starvation tail (worker-mode P99 280 ms → 58 ms at unchanged throughput, issue #442). `"barge"` restores the pre-fix racing admission (wait in the bounded channel's own `send()`, where a fresh request can steal a freed slot ahead of parked waiters) — work-conserving but unfair; operator escape hatch. Inert with the legacy `fpm_engine = "spawn_blocking"` (no ePHPm-owned admission queue; startup WARNs if `"barge"` is set there); active on the default `fpm_engine = "pool"`. An unknown value is a startup error. Startup logs the effective policy on the pool-started line. Env: `EPHPM_PHP__ADMISSION=barge`. |
