...
| `code_bundle` | string | `"off"` | **Experimental (POC).** In-memory code bundle: a transparent, read-only overlay that answers `.php` source discovery, reads, and `stat` from RAM so the code path never touches the filesystem (apps need no changes — `require`/`file_exists`/`filemtime` on ordinary paths are just answered faster; misses fall through to disk). `"off"` (**default**) = no bundle, zero overhead. `"scan"` = scan `document_root` at startup and index every `.php` file. Reclaims the Windows filesystem-metadata tax (~50 µs/`stat`); on Linux the win is small. Pair with `opcache_validate_timestamps = false` — startup **warns** if the bundle is on with validation still on. Single-docroot only; ignored (treated as `off`) under `[server] sites_dir`, with a startup warning. An unknown value (e.g. the not-yet-implemented `"file"`) is a startup error. Env: `EPHPM_PHP__CODE_BUNDLE=scan`. |
