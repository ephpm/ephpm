...
- **Unknown keys in a per-site override file** (`<[server] site_overrides_dir>/<site-key>.toml`) — a different file, not part of `ephpm.toml`. It understands `document_root`, `auto_prepend_file` and the `[preview_auth]` section (the access gate); a key it does not implement at all is ignored with a `WARN` naming the file, the site, the keys and a "did you mean" hint, because that file is written by a separately-released provisioning daemon and strict parsing would let one of its deploys take a whole fleet off its web roots. A key ePHPm *does* implement with a value it cannot honour is **not** lenient — that site refuses to serve (503) rather than fall back to serving its container. See [Virtual Hosts → Failure modes](/guides/virtual-hosts/#failure-modes-a-broken-override-takes-the-site-out-of-service).
...
| `site_overrides_dir` | path | (none) | **Multi-tenant only.** Directory of operator-supplied per-site overrides, one `<site-key>.toml` per virtual host. Keys: `document_root = "public"` (that site's web root), `auto_prepend_file = ".preview-env.php"` (a PHP file that site runs before every request — per-request mode only), and a `[preview_auth]` section that turns the [preview access gate](/roadmap/preview-access-gate/) on for that vhost (enforced on the static **and** PHP paths, fail-closed). `document_root`/`auto_prepend_file` are relative to the **site container** and refused if they resolve outside it. A file that exists but cannot be honoured takes **that one site** out of service (503) rather than falling back to serving its container, which would be wider than the operator asked for. **The directory must live outside `sites_dir`** — startup fails closed otherwise. `open_basedir` is unaffected and stays the site container. Unset disables the mechanism. See [Virtual Hosts → Per-site document root](/guides/virtual-hosts/#per-site-document-root-frameworks-with-a-public-directory) and [Per-site `auto_prepend_file`](/guides/virtual-hosts/#per-site-auto_prepend_file-bootstrap-one-site-before-every-request). |
...
| `preview` | bool | `false` | Preview-host preset for **not-production** PR-preview instances. Adds `X-Ephpm-Preview: 1` to every response, and every `[server.limits]` knob you did not set explicitly resolves to a preview default instead of "off": `max_connections = 256`, `per_ip_max_connections = 32`, `per_ip_rate = 10.0`, `per_ip_burst = 50`, `per_site_rate = 5.0`, `per_site_burst = 20`. The preset also supplies one `[php]` knob: [`overload = "shed"`](#php), so a saturated preview instance answers `503` + `Retry-After` instead of absorbing requests until the client times out. Explicit values always win — including explicit `0`, which disables that limit even under preview, and an explicit `overload = "wait"`. Startup logs exactly which values the preset supplied. Env override: `EPHPM_SERVER__PREVIEW=true`. |
...
| `trusted_hosts` | array of strings | `[]` | Allowed `Host` header values. Empty = allow all. Mismatched hosts get 421. `/_ephpm/health`, `/_ephpm/ready`, `/_ephpm/primary`, `/_ephpm/requests` (when the request timeline is enabled), and the metrics path are exempt (probes/scrapes address pods by IP). |
...
| `multi_tenant_hardening` | bool | `true` if a `[server.security]` section is present **or** `server.sites_dir` is set, else `false` | Apply the [multi-tenant confidentiality/integrity denylist preset](/guides/virtual-hosts/#multi-tenant-hardening-preset) on top of `disable_shell_exec`: `pcntl_*`, `posix_kill`/`posix_set*id`, `pfsockopen`/`fsockopen`, SysV `shm_*`/`sem_*`/`msg_*`, `opcache_reset`/`opcache_compile_file`, `dl`, `mail`, plus `mysqli.allow_persistent=0` and (when `[opcache] cluster_invalidation` is off) `opcache.restrict_api`. Composed as a **union** with any operator `disable_functions`. **Cost: persistent DB/socket connections stop working.** (`fsockopen` alone can be lifted by `network_egress_externally_managed` — see below.) **Multi-tenant only — see below.** |
| `network_egress_externally_managed` | bool | `false` | Assert that network egress is enforced **below PHP** (nftables, eBPF, systemd `IPAddressDeny`, cloud security group). When `true` and the hardening preset is active, ePHPm stops adding the **reachability-only** `fsockopen` to `disable_functions` — blocking it while `stream_socket_client`/`curl` stay open is redundant and bypassable once the kernel decides reachability. Does **not** lift `pfsockopen` or `mysqli`/`pgsql` persistence (those close a cross-tenant *persistence* leak in the shared ZTS worker pool, not a reachability one), nor any process-control/IPC/`dl`/`mail`/OPcache block. **Defaults to `false`** (safe): never inferred from multi-tenant mode, since ePHPm cannot verify the external control exists. Setting it where the preset is inactive logs a warning. **Multi-tenant only.** |
...
| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `ebpf_policy` | bool | `false` | Enable the per-vhost eBPF policy: per-thread vhost tagging, `cgroup/bind4+6` transparent sidecar port-rewrite, and `cgroup/connect4+6` per-vhost loopback authorization. Requires `[server] sites_dir` and per-request mode (worker mode is not supported). **Setting it `true` on a non-Linux platform is a hard startup error**, never a silent no-op, and it is otherwise **fail-closed**: if the programs cannot load or attach, or the port range overlaps the ephemeral range, ePHPm refuses to start rather than serve with the policy silently absent. |
...
routed to your application — in per-request and worker
...
| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `enabled` | bool | `false` | Enable native WebSocket support. Opt-in: it changes how upgrade requests route and admits long-lived connections that outlive their HTTP request. Env override: `EPHPM_SERVER__WEBSOCKET__ENABLED`. **Startup error** when combined with `[php] mode = "worker"` (WebSocket events dispatch through the per-request path), or when `[server] websocket_files` is empty. |
...
| `php_memory_limit` | string | (auto-derived) | Override the per-request `memory_limit`, taking precedence over `memory_limit` **and** the derivation. Unset → serve mode derives `(memory_budget − opcache_shm − ~64 MB overhead) / concurrency`, floored at `128 MB`; with no detectable memory budget it keeps PHP's `128M`. Dev keeps `memory_limit`. |
...
| `mode` | enum | `"per_request"` | Request-execution model. `"per_request"` (default) = php-fpm-shaped: full `php_request_startup`/`shutdown` per request on ePHPm's dedicated PHP thread pool. `"worker"` = persistent worker mode: boot the framework once per worker, loop over requests (Octane/RoadRunner model); worker-only knobs live in [`[php.worker]`](#phpworker). Any other value — including the pre-v0.9.0 `"fpm"` — is a **startup error naming the key**, never a silent fallback. Env: `EPHPM_PHP__MODE=worker`. |
| `concurrency` | usize | `0` (derive) | Max PHP requests executing concurrently — the execution-pool size in **both** modes (per-request thread pool / persistent worker pool). `0` derives it: from the cgroup CPU quota when running under one (Linux), otherwise from host parallelism — clamped `[2, 32]` on **both** paths. The floor is 2, never 1: a 1-thread pool deadlocks a PHP loopback subrequest (a script HTTP-requesting its own server holds the only thread while waiting for a response that needs that thread — #461), and `--cpus 1` is a common container shape. An explicit value is used as-is, including `1`. The pool is separate from tokio's blocking pool, so capping PHP never caps static-file serving. Startup logs the effective value and its source at INFO. Env: `EPHPM_PHP__CONCURRENCY=8`. |
| `queue_depth` | usize | `0` (= `concurrency`) | Dispatch-queue depth in front of the execution pool (both modes). A full queue applies backpressure (`overload = "wait"`) or sheds 503 (`overload = "shed"`); a starved queue becomes a 504 via the request timeout. Named `queue_depth`, not `backlog` — `backlog` collides with the TCP listen backlog an operator may also be tuning. Env: `EPHPM_PHP__QUEUE_DEPTH=64`. |
| `crash_containment` | bool | `false` | **Experimental. Per-request mode only.** Contain a PHP C-stack overflow **from a deep object-graph free** instead of letting its `SIGSEGV` abort the whole process. (Runaway *recursion* does not need this: PHP's own C-stack guard catches it at the VM's call checkpoints and raises the catchable `Error: Maximum call stack size ... reached`, which is a plain `500` on every platform and in every mode. A destructor cascade passes no such checkpoint, which is what this knob is for.) The offending request is answered `500` and the pool thread that ran it is retired and replaced. **Only stack-overflow faults are contained** — heap corruption and wild writes produce the same signal but may already have damaged shared memory, so they still die with the usual fatal-signal diagnostic. Costs: each contained crash abandons the poisoned thread's PHP context (a bounded leak), and once any crash has been contained the process skips PHP module shutdown at exit. Set in worker mode it changes nothing and startup logs a WARN. Env: `EPHPM_PHP__CRASH_CONTAINMENT=true`. |
| `overload` | string | unset (`"wait"`; `"shed"` under `[server] preview`) | What a PHP request does when no execution slot is free. `"wait"` queues and waits — historical behaviour, where the outer `[server.timeouts] request` deadline is the only bound. `"shed"` answers `503 Service Unavailable` + `Retry-After: 1` once the request has waited `shed_after_ms`, turning overload into fast, countable errors instead of client timeouts. **Unset is not the same as `"wait"`**: it resolves to `"shed"` under `[server] preview = true` (set `"wait"` explicitly to opt out). What it bounds is the dispatch queue (`queue_depth`), which always exists, so shedding works out of the box. Startup logs which policy is in force and where it came from. Ignored in worker mode (startup WARNs). Shed responses are counted by `ephpm_php_shed_total`. An unknown value is a startup error. Env: `EPHPM_PHP__OVERLOAD=shed`. |
| `shed_after_ms` | u64 (ms) | `0` | How long a request may wait for a PHP execution slot before `overload = "shed"` rejects it. `0` = do not wait at all — take a free slot if there is one, otherwise shed immediately (the dispatch queue, `queue_depth` slots, is already the buffer). Raise it to absorb bursts before shedding. Inert when the policy is `"wait"`. Env: `EPHPM_PHP__SHED_AFTER_MS=250`. |
| `admission` | string | `"fifo"` | Dispatch-admission policy for the bounded queues in front of PHP execution — the per-request pool and the worker pool. `"fifo"` (**default**) admits waiters in strict arrival order via a FIFO-fair semaphore — removes the multi-lap starvation tail (worker-mode P99 280 ms → 58 ms at unchanged throughput, issue #442). `"barge"` restores the pre-fix racing admission (wait in the bounded channel's own `send()`, where a fresh request can steal a freed slot ahead of parked waiters) — work-conserving but unfair; operator escape hatch. An unknown value is a startup error. Startup logs the effective policy on the pool-started line. Env: `EPHPM_PHP__ADMISSION=barge`. |

### `[php.worker]`

Worker-mode configuration — every knob here applies only when `[php] mode = "worker"`. The shared scheduling knobs (`concurrency`, `queue_depth`, `admission`, `overload`, `shed_after_ms`) stay on `[php]` because they bound both modes. An unknown key under `[php.worker]` — including a pre-rename top-level name like `worker_count` — fails startup naming the key.

| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `script` | path | (none) | Worker entrypoint, relative to `document_root`. **Required** when `mode = "worker"`; config load hard-errors if absent or not a file under `document_root`. Env: `EPHPM_PHP__WORKER__SCRIPT=worker.php`. |
| `max_requests` | u64 | `10000` | Recycle a worker after N requests — pure leak guard for the framework kernel. For a leak-free loop, recycling is pure overhead (framework re-boot cost); prefer `0` when you trust your kernel. Each recycle is logged at debug (worker id, requests served, uptime). |
| `boot_timeout` | u64 (sec) | `30` | Seconds a worker gets to boot and reach its first `take_request()`. A boot still running when this expires is logged as an error and counted in `ephpm_worker_boot_timeouts_total`; the thread is not killed and still becomes ready if the boot completes. (A boot that *fails* — the script exits before its first `take_request()` — is counted as a boot failure and respawned with backoff, independent of this timeout.) |
| `populate_superglobals` | bool | `false` | Populate native `$_GET`/`$_POST`/`$_SERVER`/... per request. Off for Octane/PSR-15 (they build their own request); on for the WordPress adapter. |
| `stream_threshold` | u64 (bytes) | `1048576` (1 MiB) | Request-body size at/above which the body **streams** into the worker in fixed-size chunks instead of buffering whole (Phase 3). Requests with a `Content-Length` at/above this — or with no `Content-Length` (chunked) — flow through `Envelope::bodyStream()` / PHP's POST reader with flat worker memory (a multi-GB upload never materializes in RAM). Smaller bodies stay buffered. |
...
### Migration from the pre-v0.9.0 `[php]` keys

The v0.9.0 rename removed the `spawn_blocking` execution engine (the dedicated pool is the only per-request engine) and collapsed/renamed the concurrency knobs. **Every old key is a startup error naming the key** — an old config fails fast rather than silently dropping your concurrency cap. The mapping:

| Old key | New key | Notes |
|---------|---------|-------|
| `mode = "fpm"` | `mode = "per_request"` | Same behaviour; renamed because ePHPm is not php-fpm and the old name invited `pm.*` expectations. `mode` is now an enum — a typo is a startup error. |
| `fpm_engine` | *(removed)* | The dedicated pool is the only engine. Delete the key. |
| `workers` | `concurrency` | The `spawn_blocking` semaphore this capped is gone; `concurrency` is the pool size and the cap. `workers = 0` meant *unlimited*; `concurrency = 0` means *derived* (clamped `[2, 32]`) — an unbounded PHP concurrency setting no longer exists. |
| `worker_count` | `concurrency` | Collapsed into the one knob; it sizes the worker pool in worker mode exactly as before. |
| `worker_backlog` | `queue_depth` | Same semantics, both modes. |
| `overload_policy` | `overload` | Same values (`"wait"`/`"shed"`) and preview-preset behaviour. |
| `worker_script` | `[php.worker] script` | Moved into the subsection. |
| `worker_max_requests` | `[php.worker] max_requests` | Moved. |
| `worker_boot_timeout` | `[php.worker] boot_timeout` | Moved. |
| `worker_populate_superglobals` | `[php.worker] populate_superglobals` | Moved. |
| `worker_stream_threshold` | `[php.worker] stream_threshold` | Moved. |

Also new in v0.9.0: the derived `concurrency` clamps to a floor of **2 on the cgroup-quota path too** (#461) — a `--cpus 1` container used to derive a 1-thread pool, which deadlocks a PHP loopback subrequest. Startup logs an INFO line when the clamp raises the derived value; set `concurrency = 1` explicitly to override.
...
1. **CPU quota** — cgroup v2 `cpu.max`, else v1 `cpu.cfs_quota_us`/`cpu.cfs_period_us`. `None` when unlimited. (Already drives `concurrency`.)
...
1. **CPU quota** — no cgroup equivalent is read; `concurrency` derives from host parallelism.
...
| `memory_limit` (per request) | `(budget − opcache_shm − ~64 MB overhead) / concurrency` | floor `128` MB |
...
- **ePHPm sets `opcache.cache_id` to a per-process value** (`ephpm-<pid>`). PHP names its Windows SHM section from the user name, SAPI name, build id, `opcache.cache_id`, and the requested size, in a namespace shared across the session. Two ePHPm processes that compute the same name make the second one *reattach* to the first one's segment, which PHP permits only if the VM opcode handlers sit at the same address in both images — cached opcodes store absolute handler pointers. Different binaries get different ASLR image bases, so the second process would die at startup with `Opcode handlers are unusable due to ASLR`. A per-process `cache_id` gives each process its own segment, which is how ePHPm already behaves on Linux and macOS (PHP does not support cross-process reattachment there at all). Setting your own `opcache.cache_id` through `ini_overrides` overrides this.
  ...
  **Interaction with `opcache.file_cache`.** ePHPm never sets `opcache.file_cache`, but if you enable it through `ini_overrides` on Windows, be aware that PHP also keys the file cache's on-disk directory off `opcache.cache_id` (via `md5(user ‖ cache_id)`). With the per-process `ephpm-<pid>` value this hands every process a fresh file-cache directory, so the file cache is cold on every restart and orphan directories accumulate on disk unbounded. To use a persistent file cache, pin your own stable `opcache.cache_id` next to it — accepting that you re-own the ASLR-collision risk if you run two different ePHPm images on the same box. See [Getting the Most from PHP on a Windows Box](/guides/windows-performance/) for the full caveat. (Not to be confused with `[server.file_cache]`, ePHPm's in-memory static-file cache.)
...
```
autotune (serve): cpu_quota=4.00 mem=4096MiB (cgroup v2) -> concurrency=4[cgroup_quota] opcache.memory_consumption=512MB memory_limit=880M interned=32MB jit_buffer=64MB (jit=disable) max_files=20000 realpath=16M/ttl=600 validate_timestamps=0 assertions=-1
```
...
| `mysql_wire_enabled` | bool | `true` | Whether to bind the per-site MySQL wire listener at all. Set `false` for **bridge-only** multi-tenant deployments where every app talks to its per-site database exclusively through the in-process `ephpm_db_*` SAPI bridge and nothing uses stock `pdo_mysql`. When `false`, ePHPm does not bind `mysql_listen` and injects no `DB_HOST`/`DB_PORT`/`DB_USER`/`DB_PASSWORD` — one fewer local attack surface. The per-site database registry and the `ephpm_db_*` bridge stay wired up, so in-process access is unaffected; only the wire frontend is skipped. Applies to the per-site (multi-tenant) listener. |
...
| `per_site` | bool | `false` | **Replicate per-site databases across the cluster (multi-tenant clustered mode) — EXPERIMENTAL.** Only meaningful when clustered mode is active **and** `[server] sites_dir` is set. When `true`, each virtual host's database at `<dir>/<site-key>.db` replicates to every node, so any node can serve any site. Ownership of a site is chosen by **rendezvous hashing (HRW)** over the alive nodes, so a node death moves only that node's sites — each to a node already holding a warm replica. When `false` (the default), a clustered + multi-site config gets **no** per-site isolation: every vhost shares the one clustered database, and startup warns. Ignored outside clustered multi-site mode. See the note below for the write path and its gap. |

> **Per-site clustered mode: writes are owner-served.** A request that lands on a node which does **not** own the site has its statements forwarded to the site's HRW owner over the cluster channel (already mutually authenticated and encrypted); the owner runs them against its local database, so the write is captured into CDC and replicates to every node. Reads and writes therefore both work on any node, and read-your-writes holds because one node serves both. Forwarding covers **both** tenant routes — the `ephpm_db_*` bridge and stock `pdo_mysql` on the multi-tenant wire listener. Two things to know: a connection is routed **once, when it is opened**, so if ownership moves while it is open the old owner refuses and the tenant sees a connection error until it reconnects (loud, rather than a write that would not replicate); and a forwarded statement's query stats are recorded on the **owner**, not on the node the request hit. `ATTACH` / `VACUUM` / path-`PRAGMA` screening is unchanged — it runs locally, before anything is forwarded. This mode is **experimental**: the Turso engine is Beta upstream, per-site CDC layers on top of it, and multi-node behaviour is unit-tested rather than validated on a live cluster.

> **Per-site clustered mode: every node that serves a site also replicates it.** A node joins a site's replication set — subscribing to that site's CDC stream and materializing a local replica — as soon as it serves the site at all, whether it owns the site or merely forwards to the owner. This is what makes the "each to a node already holding a warm replica" guarantee above true for the recommended bridge-only deployment. Through v0.8.6 only a *local database open* triggered replication, so a node whose apps used the `db-*` drop-ins exclusively (`ephpm_db_*`, never stock `pdo_mysql`) forwarded a site's traffic without ever replicating it: the tenant's data lived on exactly one node, invisibly, until that node died. Note that a site's replication set is **not** bounded by `[db.sqlite] max_open_dbs` — that knob bounds *serving* database handles, and an evicted serving handle never stops replication.

> **Per-site clustered mode: what an ownership move does to the data.** A site's owner is recomputed from live membership, so it changes whenever the cluster does — a node **joining** is enough, no failure required. When ownership moves to a node that holds none of the site's data, that node pulls a snapshot from the previous owner (HRW over the alive set excluding itself) before serving, so the move takes the data with it. Two rules bound the damage if that pull cannot happen: a node never discards data it originated in order to bootstrap from a peer, and a snapshot carrying no tables is refused outright rather than applied over live data — the node keeps serving what it has and retries. Ownership resolution follows live membership rather than the published claim, so a claim left behind by a membership change cannot wedge a site's replication. Multi-node ownership churn has **not** been validated on a live cluster; treat it as experimental.

> **Per-site clustered mode and `/_ephpm/primary`.** There is no cluster-wide primary in this mode, so the endpoint returns `200` on every healthy node. Ownership is per tenant and a non-owner forwards its writes to the site's owner, so every node accepts writes for every site. Use round-robin routing here; the active-passive pattern applies to single-database clustered mode. See [Kubernetes → Active-passive writes](/roadmap/kubernetes/#active-passive-writes-_ephpmprimary).
...
| Verdict: rewrite | Assign to `$_SERVER` (e.g. `$_SERVER['HTTP_X_USER']`). As in the native lane, a `REQUEST_URI` change does not re-resolve the script in per-request mode. |
...
| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `cluster_invalidation` | bool | (auto) | Watch the KV store for invalidation events. Unset defaults to `true` when `[cluster] enabled = true`, `false` otherwise. Applies to per-request mode only (`[php] mode = "per_request"`); worker mode logs a WARN at startup and skips the watcher. |
