...
- **[Turso Engine](turso-engine/)** — one engine for both modes: the Rust SQLite rewrite that replaced rusqlite and the sqld sidecar, **shipped in v0.7.0** (single-node default; clustered CDC experimental). GA labeling still tracks upstream gates.
...
- **[SSE & Realtime](sse-realtime/)** — `ephpm_kv_wait()` and streaming brotli **shipped**; the render-once/fan-out SSE hub that decouples viewer count from `concurrency` is the v0.7 target.
...
- **[Engine-Backed Code Analysis](opcode-analysis/)** — analyzing PHP with the embedded engine itself: L1 opcode-level sink detection (`opcode-scan`) **shipped, opt-in**; superglobal taint and L2 detonation are the planned next levels.
