...
The `Host:` header chooses the directory. With `[server.security] open_basedir = true` (default in vhost mode), each site's PHP is filesystem-sandboxed to its own directory. See [Architecture → Architecture](/architecture/) for the vhost design.
...
- [Apache mod_php → ePHPm migration](/migration/apache-mod-php/)
- [Nginx + php-fpm → ePHPm migration](/migration/nginx-php-fpm/)
