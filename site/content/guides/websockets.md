...
- **Not supported in worker mode.** `[server.websocket] enabled = true` with `[php] mode = "worker"` is a startup error: worker mode routes every request into the persistent worker's loop, so the entrypoint would never execute. Use per-request mode (the default).
