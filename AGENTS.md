...
- Embeds PHP via `libphp` (ZTS, thread-safe) compiled with `--enable-embed --enable-zts`. The prebuilt `libphp.a` is downloaded from `github.com/ephpm/php-sdk` releases (built upstream by static-php-cli; ePHPm itself does not invoke spc).
...
| Static PHP builds | `github.com/ephpm/php-sdk` releases — prebuilt `libphp.a` + headers downloaded by xtask (built upstream via static-php-cli) |
...
- `libphp.a` is downloaded as a prebuilt artifact from `github.com/ephpm/php-sdk` releases (PHP 8.3, 8.4, and 8.5) by `cargo xtask release`
- FFI bindings generated by `bindgen` in `build.rs`
- Custom SAPI in `ephpm_wrapper.c` handles the full request lifecycle
- HTTP server serves PHP scripts via `ephpm serve` (HTTP/1.1 + HTTP/2)
- `ephpm dev` runs a zero-config dev server with `--sites` vhosting and `*.localhost` routing
...
```
crates/
├── ephpm/              # Binary crate — CLI (clap), `serve`/`dev`/`admin`/`php`/service subcommands,
│                       # config loading, server boot, graceful shutdown
├── ephpm-config/       # figment-based config (TOML + EPHPM_ env vars, __ nesting separator)
├── ephpm-php/          # PHP embedding via FFI — SAPI implementation, ZTS thread model,
│                       # ephpm_wrapper.c bridge, native session handler
├── ephpm-server/       # HTTP server (hyper + tokio) — routing, TLS/ACME, static files,
│                       # metrics, litewire/SQLite startup, TrackedBackend query stats
├── ephpm-db/           # In-process SQL proxy — MySQL wire protocol, pooling, R/W splitting
├── ephpm-kv/           # In-process KV store — DashMap, RESP, TTL, compression, SETNX,
│                       # PHP SAPI bindings
