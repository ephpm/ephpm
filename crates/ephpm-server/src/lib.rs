...
async fn start_db_proxies(config: &Config) -> anyhow::Result<Vec<tokio::task::JoinHandle<()>>> {
    ...
    // LiteWire (embedded SQLite with MySQL/Hrana wire frontends)
    #[cfg(feature = "litewire")]
    if let Some(lw_config) = &config.db.litewire {
        // Mutual exclusivity: litewire and mysql proxy can't both bind the same port.
        if let Some(mysql_config) = &config.db.mysql {
            let mysql_listen = mysql_config
                .listen
                .as_deref()
                .unwrap_or("127.0.0.1:3306");
            let lw_listen = lw_config
                .mysql_listen
                .as_deref()
                .unwrap_or("127.0.0.1:3306");
            if mysql_listen == lw_listen {
                anyhow::bail!(
                    "db.mysql.listen ({mysql_listen}) and db.litewire.mysql_listen \
                     ({lw_listen}) conflict — only one can bind a given port"
                );
            }
        }

        let handle = start_litewire(lw_config)
            .await
            .context("failed to start LiteWire")?;
        handles.push(handle);
    }

    #[cfg(not(feature = "litewire"))]
    if config.db.litewire.is_some() {
        tracing::warn!(
            "db.litewire is configured but the `litewire` feature is not enabled — \
             rebuild with `--features litewire`"
        );
    }
...
/// Build and spawn LiteWire frontends.
#[cfg(feature = "litewire")]
async fn start_litewire(
    config: &ephpm_config::LiteWireConfig,
) -> anyhow::Result<tokio::task::JoinHandle<()>> {
    use litewire::LiteWire;

    let builder = match config.backend.as_str() {
        "rusqlite" => {
            let db = litewire::backend::Rusqlite::open(&config.path)
                .map_err(|e| anyhow::anyhow!("{e}"))
                .with_context(|| format!("failed to open SQLite database: {}", config.path))?;
            tracing::info!(path = %config.path, "LiteWire rusqlite backend opened");
            LiteWire::new(db)
        }
        other => {
            anyhow::bail!(
                "unknown litewire backend: {other} (expected \"rusqlite\")"
            );
        }
    };

    let builder = if let Some(ref addr) = config.mysql_listen {
        tracing::info!(listen = %addr, "LiteWire MySQL frontend enabled");
        builder.mysql(addr)
    } else {
        builder
    };

    let builder = if let Some(ref addr) = config.hrana_listen {
        tracing::info!(listen = %addr, "LiteWire Hrana frontend enabled");
        builder.hrana(addr)
    } else {
        builder
    };

    // Inject environment variables for PHP framework auto-discovery.
    if config.inject_env {
        if let Some(ref addr) = config.mysql_listen {
            inject_litewire_env(addr)
                .context("failed to inject LiteWire DB env")?;
        }
    }
...
/// Set DB environment variables so PHP frameworks auto-discover LiteWire.
#[cfg(feature = "litewire")]
fn inject_litewire_env(listen: &str) -> anyhow::Result<()> {
    let addr: std::net::SocketAddr = listen
        .parse()
        .with_context(|| format!("invalid listen address for inject_env: {listen}"))?;

    // SAFETY: called before PHP requests start (single-threaded init phase).
    unsafe {
        std::env::set_var("DB_HOST", addr.ip().to_string());
        std::env::set_var("DB_PORT", addr.port().to_string());
        std::env::set_var("DB_CONNECTION", "mysql");
        std::env::set_var("DB_DATABASE", "litewire");
        std::env::set_var("DB_USERNAME", "root");
        std::env::set_var("DB_PASSWORD", "");
        std::env::set_var(
            "DATABASE_URL",
            format!("mysql://root@{}:{}/litewire", addr.ip(), addr.port()),
        );
    }
    ...
    tracing::debug!(
        host = %addr.ip(),
        port = %addr.port(),
        "injected LiteWire DB environment variables"
    );
