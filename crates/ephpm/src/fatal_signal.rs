...
/// so a genuine fatal (heap corruption, a null deref, a fault outside a guarded
...
/// [`ephpm_php::crash_guard::recover_hook`] here, and only when
/// `[php] crash_containment` is active — with no hook registered this module
/// behaves exactly as it did before containment existed.
///
/// The last registration wins. On Windows this is a no-op.
...
mod unix {
    ...
    /// `unsafe extern "C" fn(usize) -> i32` address (`0` = none, the default).
    /// Consulted first thing in [`handler`] for SIGSEGV/SIGBUS. An atomic rather
    /// than a lock because it is read from a signal handler.
    ...
    extern "C" fn handler(sig: c_int, info: *mut siginfo_t, ctx: *mut c_void) {
        ...
        // OWNER gate so a contained fault never touches the shared handler state
        // and cannot interfere with a concurrent genuine fatal on another
        // thread. No hook is registered unless `[php] crash_containment` is
        // active, so by default this is one relaxed load on a dying process.
        if matches!(sig, libc::SIGSEGV | libc::SIGBUS) && !info.is_null() {
            let hook = RECOVER_HOOK.load(Ordering::Acquire);
            if hook != 0 {
                // SAFETY: `RECOVER_HOOK` is either 0 or a valid
                // `unsafe extern "C" fn(usize) -> i32` stored by
                // `set_recover_hook`, whose contract requires it to be
                // async-signal-safe. It may not return — it siglongjmps on a
                // contained stack overflow — which is why nothing above it has
                // taken a lock or claimed the OWNER gate yet.
                let hook: unsafe extern "C" fn(usize) -> i32 = unsafe {
                    std::mem::transmute::<usize, unsafe extern "C" fn(usize) -> i32>(hook)
                };
                // SAFETY: as above — calling the registered async-signal-safe
                // classifier with this fault's address.
