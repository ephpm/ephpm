...
Stock `pdo_mysql` also works per-site (`crates/ephpm-server/src/site_wire_auth.rs`). **One** MySQL listener serves every tenant; a connection's database is fixed by the credential it authenticates with, not by anything it claims. `DB_USER` is the site key, `DB_PASSWORD` is `HMAC-SHA256(master_secret, site_key)` — the same derivation the KV RESP listener uses (`ephpm_kv::auth::derive_site_password`) — injected into that site's `$_SERVER` per request by the router. Verification happens *before* the backend is resolved, so a failed auth never opens the target's database file.

**The wire-auth master secret is config-sourced (`[kv] secret`) when set (#471).** `SiteWireAuth` seeds from `[kv] secret` — the same secret the KV RESP listener already uses — falling back to a fresh per-process random secret when it is unset. With a stable secret the per-site passwords are stable across restarts, which is what lets a co-located CLI (`ephpm php --site`, see below) authenticate to the listener as the site. **Security implication, stated plainly: with `[kv] secret` set, anyone who can read the config file can derive any tenant's password and impersonate that tenant** — every password is a pure function of the secret and the (public) site name. This is the same trust boundary `ephpm exec` documents ("whoever can read this config can already impersonate any tenant"). Without `[kv] secret` the server still runs (random secret, credentials rotate per restart) and the CLI wire path is simply unavailable (it fails closed with a clear message). The secret is never sent to PHP and never logged; only per-site derivations are injected into `$_SERVER`.

**`ephpm php --site <key> --config <path>` (#471)** binds the in-process `ephpm_db_*` bridge to a vhost's database so `wp`/`artisan`/migrations (the `db-*` Composer packages) work from the CLI. A startup strategy picker (`crates/ephpm/src/site_db.rs`) chooses: if the server's MySQL listener answers, a `RemoteBackend` forwards **raw** SQL over the wire authenticating as the site (the server does the single translation/screening/owner-forwarding — correct in per-site *clustered* mode and while the server holds the file open); if no server is running, it direct-opens the site's own Turso file for offline seeding (single-node/per-site-single only); per-site-clustered with no server is **refused**. `resolve_sandbox_site` validates the key fail-closed, and `screen_sql` runs CLI-side on every statement. The Remote path deliberately does **not** reuse the bridge's translating `Session` (that would translate MySQL→SQLite twice — once here, once at the server); it registers via `db_bridge::BackendSource::Remote` / `set_remote_backend`, which forwards verbatim. Per-site listeners were rejected deliberately: all tenants share one process and one uid, so neither a port (enumerable) nor a unix socket (identical permissions; `open_basedir` does not gate `unix://` or PDO's `unix_socket`) can separate them — see the module docs. Hrana/PG/TDS stay off in multi-site mode (they cannot bind a backend per connection). `ATTACH`/`DETACH`/`VACUUM`/path-`PRAGMA` are rejected on the tenant path in all modes.
...
- **PHP threading**: ZTS (Zend Thread Safety) is implemented. PHP is compiled with `--enable-zts` and each thread auto-registers with TSRM on first use, getting its own isolated PHP context. PHP always executes on threads ePHPm owns: per-request mode (`[php] mode = "per_request"`, the default) runs a fixed pool of dedicated OS threads (`crates/ephpm-server/src/fpm_pool.rs`) sized by `[php] concurrency` (0 = derived from cgroup quota / host parallelism, clamped [2, 32]); worker mode runs the persistent worker pool. The old `spawn_blocking` execution engine (and its `[php] workers` semaphore) was **removed** — tokio's blocking pool never runs PHP, so slow scripts cannot starve static-file serving by construction. A `Mutex` protects only one-time `init()`/`shutdown()`, not request execution. An `AtomicBool` fast-path check avoids the mutex for the common "is PHP ready?" path. Per-request C statics use `__thread` for thread isolation. **Windows is ZTS too** — the php-sdk's static `php8embed.lib` is a ZTS build (TSRM exports, `_tsrm_ls_cache`; `ephpm php -v` reports ZTS) and the wrapper/bindgen compile with `ZTS=1` (issue #326; the old "Windows is NTS" claim was wrong). What Windows genuinely lacks: per-thread execution timers (`ZEND_MAX_EXECUTION_TIMERS` — so `max_execution_time` is not natively enforced; the `[server.timeouts] request` ceiling still applies) and the Unix-only stack-overflow crash containment (`crash_guard.c`).
