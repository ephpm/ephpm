...
/// image) with two changes: (a) listen ports are per-node so multiple ephpm
/// instances can co-exist on the same host, (b) `document_root` points at the
/// checkout's `tests/docroot` instead of `/var/www/html`.
...
impl SingleNodeSpawn {
    fn start(binary: &Path, docroot: &Path, scratch_root: &Path) -> io::Result<Self> {
        ...
        let mysql_port: u16 = 13306;
        let hrana_port: u16 = 18101;
        let pg_port: u16 = 15432;
        let tds_port: u16 = 11433;
        ...
        Ok(Self { child, http_port, _data_dir: data_dir, stderr_log, stdout_log })
    }
    ...
    fn env(&self, php_version: &str) -> Vec<(String, String)> {
        vec![
            ...
            ("EPHPM_SERVER__PHP_ETAG_CACHE__ENABLED".to_string(), "true".to_string()),
        ]
    }
