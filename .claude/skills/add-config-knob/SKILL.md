...
7. **Tests**: default value test in ephpm-config; behavior test where enforced. A test that needs an `EPHPM_*` override must install it with `crate::test_env::EnvVars` - never `std::env::set_var` directly. An unguarded environment mutation is invisible to a green run and shows up later as an unrelated test flaking under the parallel harness (issue #235).
